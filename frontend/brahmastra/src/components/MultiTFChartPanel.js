      ...
      {(!data || data.length === 0) ? (
        ...
        <ResponsiveContainer width="100%" height={150}>
          <ComposedChart data={data} margin={{ top: 4, right: 6, bottom: 0, left: -18 }}>
            ...
            <XAxis dataKey="t" tick={{ fontSize: 8, fill: C.dim }} minTickGap={28} />
            <YAxis domain={['auto', 'auto']} tick={{ fontSize: 8, fill: C.dim }} width={44} />
            <Tooltip contentStyle={{ background: C.panel2, border: `1px solid ${C.border}`, fontSize: 11 }}
                     labelStyle={{ color: C.dim }} />
            <Line type="monotone" dataKey="bbU" stroke={C.amber} dot={false} strokeWidth={1} strokeDasharray="3 3" isAnimationActive={false} name="BB upper" />
            <Line type="monotone" dataKey="bbM" stroke={C.dim}   dot={false} strokeWidth={1} isAnimationActive={false} name="BB mid" />
            <Line type="monotone" dataKey="bbL" stroke={C.amber} dot={false} strokeWidth={1} strokeDasharray="3 3" isAnimationActive={false} name="BB lower" />
            <Line type="monotone" dataKey="close" stroke={C.cyan} dot={false} strokeWidth={1.6} isAnimationActive={false} name="Close" />
          </ComposedChart>
        </ResponsiveContainer>
      )}
    </div>
...
const S = {
  ...
  title: { fontSize: 12, fontWeight: 700, letterSpacing: 1, color: C.text },
  instBtn: { background: 'transparent', border: `1px solid ${C.border}`, color: C.dim, fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 3, cursor: 'pointer' },
  instBtnOn: { borderColor: C.cyan, color: C.cyan },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 8 },
  chartBox: { background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 6, padding: 6 },
  chartHead: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 2 },
  empty: { height: 150, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.dim, fontSize: 11 },
};
